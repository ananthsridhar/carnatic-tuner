...
export function TunerUtil() {
  ...
  return {
    ...
    findCentsOffPitch: function (freq, refFreq) {
    ...
    detectPitch: () => {
      ...
      var fundalmentalFreq = this.findFundamentalFreq(
        buffer,
        audioContext.sampleRate
      );

      if (fundalmentalFreq !== -1) {
        var note = this.findClosestNote(fundalmentalFreq, notesArray);
    ...
    //   frameId = window.requestAnimationFrame(this.detectPitch);
    },
...
//   };
  ...
  //         getUserMedia({ audio: true }).then(thisstreamReceived).catch(reportError);
...
//       this.updatePitch(notesArray[currentNoteIndex].frequency);
//       this.updateNote(notesArray[currentNoteIndex].note);
//       isRefSoundPlaying = true;
//     } else {
//       this.turnOffReferenceSound();
//     }
//   };
